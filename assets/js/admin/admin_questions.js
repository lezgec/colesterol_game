...
async function submitGeneratorForm(event) {
    ...
    try {
        const url = mode === "bulk"
            ? "/colesterol_game/backend/questions/generate_questions_bulk.php"
            : "/colesterol_game/backend/questions/generate_question.php";
...
function setupCsvImport() {
    ...
    csvForm.addEventListener("submit", async event => {
        ...
        try {
            const response = await fetch("/colesterol_game/backend/questions/import_questions_csv.php", {
                method: "POST",
                body: formData
            });
            const result = await response.json();
