...
CREATE DATABASE IF NOT EXISTS `colesterol_game_db`
...
CREATE TABLE `questions` (
  ...
  KEY `idx_questions_created_by` (`created_by_user_id`),
  KEY `idx_questions_reviewed_by` (`global_reviewed_by`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `game_rooms` (
  ...
  `initial_difficulty` decimal(3,1) NOT NULL DEFAULT 1.0,
...
CREATE TABLE `room_question_requirements` (
  ...
  `difficulty_level` decimal(3,1) NOT NULL,
...
ALTER TABLE `questions`
  ...
  ON DELETE SET NULL,
  ADD CONSTRAINT `fk_questions_global_reviewed_by`
  FOREIGN KEY (`global_reviewed_by`) REFERENCES `users` (`id`)
  ON DELETE SET NULL;
